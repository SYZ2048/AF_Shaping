...
Waveform_Library = cell(1, num_waveforms);
...
% 初始化随机波形和目标函数值 
s_random = exp(1j * 2*pi * rand(N_len,1)); 
plot_ambiguity_function(s_random, N_len, Nv, zeros(N_len, Nv+1), vh_range, fs);
...
for i = 1 : num_waveforms
    current_target_freq = library_dopplers(i);
    disp(['正在生成波形 ', num2str(i), '/', num2str(num_waveforms), ...
          ' (目标多普勒: ', num2str(current_target_freq), ' Hz)']);

    [~, freq_bin_idx] = min(abs(doppler_bins - current_target_freq));  % 找最近的bin
    % 设置干扰区域(p_k) (bin从0开始, MATLAB索引从1开始)
    interference_map = zeros(N_len, Nv+1);
    interference_map(1:N_len, freq_bin_idx:freq_bin_idx) = 1;   % Q
    interference_map(20:N_len, Nv/2+1) = 0.8;
